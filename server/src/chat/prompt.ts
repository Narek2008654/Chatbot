...
1. INTERVIEW the user briefly (one question at a time) about the agent they want:
   ...
   - TWO NO-PICKUP SMS templates (the call doesn't connect: no answer, busy, voicemail, dial failed). Ask for both:
     ...
     Both must be short (1-2 sentences) and use ONLY these placeholders (the SAME ones the agent prompt uses): {{caller_name}}, {{position}}, {{position_details}}, {{company_name}}, {{caller_context}}, {{call_reason}}. Do NOT invent new names like {{job_title}} or {{job_description}} — they won't fill. If the user is unsure, propose sensible defaults and let them confirm or edit.
